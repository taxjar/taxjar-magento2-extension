...
use Taxjar\SalesTax\Observer\SalesOrderSyncObserver;
use Taxjar\SalesTax\Test\Unit\UnitTestCase;

class SalesOrderSyncObserverTest extends UnitTestCase
{
    /**
    ...
    public function testExecute()
    {
        ...
        $orderMock->expects(static::once())->method('getId')->willReturn(123);
        $orderMock->expects(static::once())->method('getState')->willReturn('valid');

        $eventMock = $this->getMockBuilder(Event::class)->addMethods(['getOrder'])->getMock();
        $eventMock->expects(static::once())->method('getOrder')->willReturn($orderMock);
    ...
    {
        $orderMock = $this->createMock(Order::class);
        $orderMock->expects(static::once())->method('getId')->willReturn(null);
    ...
    {
        $this->sut = new SalesOrderSyncObserver(
