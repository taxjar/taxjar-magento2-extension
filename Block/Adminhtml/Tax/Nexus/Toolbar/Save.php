...
/**
 * Admin nexus save toolbar
 */
namespace Taxjar\SalesTax\Block\Adminhtml\Tax\Nexus\Toolbar;
...
class Save extends \Magento\Backend\Block\Template implements \Magento\Backend\Block\Widget\ContainerInterface
{
    ...
    {
        parent::_construct();
        $this->assign('createUrl', $this->getUrl('taxjar/nexus/save'));
    }
    ...
    protected function _prepareLayout()
    {
        ...
        $address = intval($this->getRequest()->getParam('address'));
        if ($address) {
            $this->buttonList->add(
                'delete',
                [
                    'label' => __('Delete Nexus Address'),
        ...
        $this->buttonList->add(
            'save',
            [
                'label' => __('Save Nexus Address'),
                'class' => 'save primary save-tax-class',
                'onclick' => 'jQuery("#nexusForm").submit()'
            ]
        );
